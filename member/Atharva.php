<!DOCTYPE html>
<?php include "./menu.php"; ?>
<?php require "../path.php"; ?>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Atharva Paranjape</title>
    <link href="../static/bootstrap-5.0.2-dist/css/bootstrap.min.css" rel="stylesheet" type="text/css">
    <script src="../static/bootstrap-5.0.2-dist/popperjs/popper.min.js" type="text/javascript"></script>
    <script src="../static/bootstrap-5.0.2-dist/popperjs/jquery-3.2.1.slim.min.js" type="text/javascript"></script>
    <script src="../static/bootstrap-5.0.2-dist/js/bootstrap.min.js" type="text/javascript"></script>
    <link rel="stylesheet" href="../static/css/member.css" type="text/css">
    <link rel="shortcut icon" href="../static/image/logo.png">
</head>

<body>

    <div class="container">
        <div class="member">
            <div class="bg-img">
                <div class="row">
                    <div class="col-sm">
                        <img src="../image/members/img_Atharva Paranjape.jpg" alt="Atharva Paranjape">
                    </div>
                </div>
            </div>
            <div class="info">
                <div class="row">
                    <div class="col-sm">
                        <div class="name">Atharva Paranjape, <span>BS-MS</span></div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm email">
                        <div class="base">Email</div>
                    </div>
                    <div class="col-sm email">
                        <div class="value"><a href="mailto:pa17ms058@iiserkol.ac.in,">pa17ms058@iiserkol.ac.in,</a></div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm position">
                        <div class="base">Curent position</div>
                    </div>
                    <div class="col-sm position">
                        <div class="value">BS-MS Project
</div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm webpage">
                        <div class="base">Webpage</div>
                    </div>
                    <div class="col-sm webpage">
                        <div class="value"><a href="https://students.iiserkol.ac.in/~pa17ms058">pa17ms058</a></div>
                    </div>
                    <div class="row">
                        <div class="col-sm">
                            <div class="fill">Atharva is currently pursuing Integrated BS-MS Dual Degree program at the Department of Physical Sciences, INSPIRE  Fellow. His research interests include orbital angular momentum and polarization to probe the properties of turbid media.</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>

</html>