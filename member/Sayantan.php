<!DOCTYPE html>
<?php include "./menu.php"; ?>
<?php require "../path.php"; ?>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sayantan Das</title>
    <link href="../static/bootstrap-5.0.2-dist/css/bootstrap.min.css" rel="stylesheet" type="text/css">
    <script src="../static/bootstrap-5.0.2-dist/popperjs/popper.min.js" type="text/javascript"></script>
    <script src="../static/bootstrap-5.0.2-dist/popperjs/jquery-3.2.1.slim.min.js" type="text/javascript"></script>
    <script src="../static/bootstrap-5.0.2-dist/js/bootstrap.min.js" type="text/javascript"></script>
    <link rel="stylesheet" href="../static/css/member.css" type="text/css">
    <link rel="shortcut icon" href="../static/image/logo.png">
</head>

<body>

    <div class="container">
        <div class="member">
            <div class="bg-img">
                <div class="row">
                    <div class="col-sm">
                        <img src="../image/members/img_Sayantan Das.jpg" alt="Sayantan Das">
                    </div>
                </div>
            </div>
            <div class="info">
                <div class="row">
                    <div class="col-sm">
                        <div class="name">Sayantan Das, <span>BS-MS</span></div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm email">
                        <div class="base">Email</div>
                    </div>
                    <div class="col-sm email">
                        <div class="value"><a href="mailto:sd16ms128@iiserkol.ac.in,">sd16ms128@iiserkol.ac.in,</a></div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm position">
                        <div class="base">Curent position</div>
                    </div>
                    <div class="col-sm position">
                        <div class="value">Project Student</div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm webpage">
                        <div class="base">Webpage</div>
                    </div>
                    <div class="col-sm webpage">
                        <div class="value"><a href="https://students.iiserkol.ac.in/~hs16ms002">hs16ms002</a></div>
                    </div>
                    <div class="row">
                        <div class="col-sm">
                            <div class="fill">Sayantan currently pursuing Integrated BS-MS Dual Degree program at the Department of Physical Sciences, IISER-K Fellow. His research interests lies on understanding the dispersion charectistics of orbital angular momentum carrying beams in plasmonic waveguides.</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>

</html>